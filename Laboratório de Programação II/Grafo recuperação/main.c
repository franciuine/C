...
int main(){
	...
	do{
		...
		switch(op){
			...
			case 2:
				if(cf>=1){
					printf("| Digite peso da aresta:\n*Para trabalhar com arestas sem peso, insira '1' para todas as arestas no grafo sendo trabalhado\n| ");
					scanf("%i", &peso)
					printf("| 1 Orientado\n");
					...
					switch(opp){
						case 1:
							...
							scanf("%i %i", &va, &vb);
							orientado=1;
							g = InsereArestaOrientado(g, va, vb, peso);
							cf=2;
							break;
						case 2:
							printf ("\n| Defina entre quais vértices deseja inserir aresta: \n| ");
							scanf("%i %i", &va, &vb);
							g = InsereArestaNaoOrientado(g, va, vb, peso);
							orientado=0;
							cf=2;
							break;
						default:
